...
async def play_file_handler(client, message: Message):
    ...
    status_message = await message.reply_text(f"🎬 Preparing streaming for <code>{safe_file_name}</code>...", quote=True, parse_mode=ParseMode.HTML)
    ...
    try:
        ...
        if error_code == '404':
            await status_message.edit_text(f"❌ <b>Error:</b> File not found: <code>{safe_file_name}</code>", parse_mode=ParseMode.HTML)
        else:
