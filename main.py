...
user_file_cache = {}  # Stores user_id -> {file_id -> file_name}
...
def ultra_pyrogram_progress_callback(current, total, message, start_time, task):
    ...
    try:
        if not hasattr(ultra_pyrogram_progress_callback, 'last_edit_time') or time.time() - ultra_pyrogram_progress_callback.last_edit_time > 1.5:
            ...
            # Fixed the f-string by removing the backslash
            text = (
...
def generate_streaming_url(file_name, user_id):
    """Generate a streaming URL for online playback"""
    user_file_name = f"{get_user_folder(user_id)}/{file_name}"
    
    # Generate a presigned URL that allows streaming
...
def get_video_player_links(stream_url, file_name):
    """Generate links for various video players"""
    encoded_url = quote(stream_url, safe='')
    
    # VLC Player
    vlc_link = f"vlc://{encoded_url}"
    
    # MX Player
    mx_link = f"intent:{stream_url}#Intent;package=com.mxtech.videoplayer.ad;end"
    
    # Browser players
    jwplayer_link = f"https://jwplayer.com/watch/?source={encoded_url}"
    flowplayer_link = f"https://flowplayer.com/player/?src={encoded_url}"
    
    return {
        'vlc': vlc_link,
        'mx': mx_link,
        'jwplayer': jwplayer_link,
        'flowplayer': flowplayer_link
    }

def store_file_reference(user_id, file_name):
    """Store file reference with a short ID to avoid long callback data"""
    if user_id not in user_file_cache:
        user_file_cache[user_id] = {}
    
    # Create a short hash-based ID
    file_id = str(abs(hash(file_name)) % 10000)  # 4-digit ID
    user_file_cache[user_id][file_id] = file_name
    
    # Clean up old entries (keep only last 20 files per user)
    if len(user_file_cache[user_id]) > 20:
        # Remove the oldest entry
        oldest_key = next(iter(user_file_cache[user_id].keys()))
        del user_file_cache[user_id][oldest_key]
    
    return file_id

def get_file_from_reference(user_id, file_id):
    """Get file name from stored reference"""
    if user_id in user_file_cache and file_id in user_file_cache[user_id]:
        return user_file_cache[user_id][file_id]
    return None

async def find_user_file(user_id, file_name):
    """Find a file in user's storage with fuzzy matching"""
    try:
        user_prefix = f"{get_user_folder(user_id)}/"
        ...
        user_files = [obj['Key'].replace(user_prefix, "") for obj in response['Contents']]
        
        # Exact match
        if file_name in user_files:
            return file_name
        
        # Fuzzy matching - try to find similar files
        file_name_lower = file_name.lower()
        for user_file in user_files:
            if file_name_lower in user_file.lower() or user_file.lower() in file_name_lower:
                return user_file
        
        # Try matching by extension or partial name
        for user_file in user_files:
            if (os.path.splitext(file_name)[1] and 
                os.path.splitext(file_name)[1] == os.path.splitext(user_file)[1]):
                return user_file
...
async def start_command(client, message: Message):
    ...
    await message.reply_photo(
        ...
        caption="🚀 <b>ULTRA TURBO CLOUD STORAGE BOT</b>\n\n"
                ...
                "🎬 <b>To stream online:</b> Use <code>/play &lt;file_name&gt;</code>\n\n"
...
async def play_file_handler(client, message: Message):
    """Handle online streaming of files"""
    # Check authorization
    if not await is_authorized(message.from_user.id):
        await message.reply_text("❌ Unauthorized access.")
        return
    ...
    try:
        ...
        try:
            ...
            if e.response['Error']['Code'] == '404':
                # File not found, try fuzzy matching
                found_file = await find_user_file(message.from_user.id, file_name)
                if found_file:
                    ...
                    await status_message.edit_text(
                        f"❌ <b>File not found:</b> <code>{safe_file_name}</code>\n\n"
                        f"Use <code>/list</code> to see your available files.",
                        parse_mode=ParseMode.HTML
                    )
                    return
            else:
        ...
        stream_url = generate_streaming_url(file_name, message.from_user.id)
        
        # Generate player links
        player_links = get_video_player_links(stream_url, file_name)
        
        # Store file reference for callback
        file_id = store_file_reference(message.from_user.id, file_name)
        
        # Create inline keyboard with player options
        keyboard = InlineKeyboardMarkup([
            [InlineKeyboardButton("🎬 VLC Player", url=player_links['vlc'])],
            [InlineKeyboardButton("📱 MX Player", url=player_links['mx'])],
            [InlineKeyboardButton("🌐 JW Player", url=player_links['jwplayer'])],
            [InlineKeyboardButton("🖥️ Flow Player", url=player_links['flowplayer'])],
            [InlineKeyboardButton("⬇️ Download File", callback_data=f"dl_{file_id}")]
        ])
        ...
        await status_message.edit_text(
            ...
            f"<b>Choose your player:</b>\n"
            f"• VLC Player (Recommended)\n"
            f"• MX Player (Android)\n"
            f"• JW Player (Browser)\n"
            f"• Flow Player (Browser)\n\n"
            f"<b>Direct URL:</b>\n<code>{escape_html(stream_url[:100])}...</code>",
            parse_mode=ParseMode.HTML,
...
async def handle_download_callback(client, callback_query):
    ...
    file_id = callback_query.data.replace("dl_", "")
    ...
    file_name = get_file_from_reference(user_id, file_id)
    if not file_name:
        await callback_query.answer("❌ File reference expired. Please use /download command.", show_alert=True)
        return
    ...
    await download_file_handler(client, message, file_name)

async def download_file_handler(client, message, file_name=None):
...
async def upload_file_handler(client, message: Message):
    ...
    try:
        ...
        # Store file reference for callback
        file_id = store_file_reference(message.from_user.id, os.path.basename(file_path))
        
        # Create inline keyboard with options
        keyboard = InlineKeyboardMarkup([
            [InlineKeyboardButton("🎬 Stream Online", callback_data=f"pl_{file_id}")],
            [InlineKeyboardButton("⬇️ Download Again", callback_data=f"dl_{file_id}")]
        ])
...
@app.on_callback_query(filters.regex(r"^pl_"))
async def handle_play_callback(client, callback_query):
    """Handle play button clicks from upload completion"""
    file_id = callback_query.data.replace("pl_", "")
    user_id = callback_query.from_user.id
    ...
    file_name = get_file_from_reference(user_id, file_id)
    if not file_name:
        await callback_query.answer("❌ File reference expired. Please use /play command.", show_alert=True)
        return
    
    await callback_query.answer("Preparing streaming...")
    
    # Trigger play process
    message = callback_query.message
    await play_file_handler(client, message, file_name)

async def play_file_handler(client, message, file_name=None):
    """Handle online streaming of files (internal version)"""
    if file_name is None:
        if len(message.command) < 2:
            await message.reply_text("Usage: <code>/play &lt;file_name&gt;</code>", parse_mode=ParseMode.HTML)
            return
    ...
    try:
        ...
        error_msg = escape_html(str(e))
        await message.edit_text(f"❌ <b>An error occurred:</b> {error_msg}", parse_mode=ParseMode.HTML)
