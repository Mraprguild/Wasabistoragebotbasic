...
def generate_player_links(file_name, presigned_url):
    ...
    mx_player_url = f"intent://{presigned_url}#Intent;package=com.mxtech.videoplayer.ad;scheme=http;type=video;end"
    
    # VLC Player URL
    vlc_player_url = f"vlc://{presigned_url}"
...
async def start_command(client, message: Message):
    ...
    await message.reply_photo(
        ...
        caption="🚀 <b>ULTRA TURBO CLOUD STORAGE BOT</b>\n\n"
                ...
                "🎬 <b>Player Support:</b> MX Player, VLC, Online Player & Direct Download\n\n"
...
async def upload_file_handler(client, message: Message):
    ...
    try:
        ...
        keyboard = create_player_keyboard(safe_file_name, presigned_url)
...
async def play_file_handler(client, message: Message):
    """Generate player links for files in Wasabi"""
    # Check authorization
    if not await is_authorized(message.from_user.id):
        await message.reply_text("❌ Unauthorized access.")
        return
    ...
    if len(message.command) < 2:
        await message.reply_text("Usage: <code>/play &lt;file_name_in_wasabi&gt;</code>", parse_mode=ParseMode.HTML)
        return
    ...
    status_message = await message.reply_text(f"🔍 Generating player links for <code>{safe_file_name}</code>...", quote=True, parse_mode=ParseMode.HTML)
    ...
    try:
        ...
        presigned_url = s3_client.generate_presigned_url('get_object', Params={'Bucket': WASABI_BUCKET, 'Key': user_file_name}, ExpiresIn=86400) # 24 hours
        
        # Create player keyboard
        keyboard = create_player_keyboard(file_name, presigned_url)
        
        await status_message.edit_text(
            f"🎬 <b>PLAYER LINKS READY!</b>\n\n"
            f"<b>📁 File:</b> <code>{safe_file_name}</code>\n"
