...
class DatabaseStorage(Storage):
    """Custom database storage for Pyrogram sessions"""
    ...
    async def open(self):
        ...
        cursor = self.conn.execute(
            'SELECT dc_id, auth_key, test_mode, date, user_data FROM sessions WHERE name = ?',
            (self.name,)
        )
        row = cursor.fetchone()
        ...
        if row:
            self.dc_id = row[0]
            self.auth_key = row[1] if row[1] else b""
            self.test_mode = bool(row[2])
            self.date = row[3] if row[3] else 0
            self.user_data = json.loads(row[4]) if row[4] else {}
        else:
            # New session
            self.dc_id = 2
            self.auth_key = b""
    ...
    async def save(self):
        ...
        self.conn.execute('''
            ...
            (name, dc_id, auth_key, test_mode, date, user_data)
            VALUES (?, ?, ?, ?, ?, ?)
        ''', (
            self.name,
            self.dc_id,
            ...
            int(self.test_mode),
            self.date,
            json.dumps(self.user_data) if self.user_data else None
        ))
        self.conn.commit()
    ...
    async def delete(self):
        ...
        self.conn.execute('DELETE FROM sessions WHERE name = ?', (self.name,))
        self.conn.commit()
...
class TelegramFileBot:
    ...
    def __init__(self, use_db_storage=False):
        ...
        if use_db_storage:
            self.db_conn = sqlite3.connect('sessions.db', check_same_thread=False)
            logger.info("Using database session storage")
        ...
        if use_db_storage and self.db_conn:
            client_config["workdir"] = None  # Don't use file-based storage
        else:
            client_config["workdir"] = "./session"
        
        self.app = Client("file_bot", **client_config)
    ...
    def _setup_handlers(self):
        ...
        @self.app.on_message(filters.command("session"))
        async def session_handler(client, message):
            await self.handle_session_command(client, message)
        
        @self.app.on_message(filters.command("logout"))
        async def logout_handler(client, message):
            await self.handle_logout_command(client, message)
...
☁️ Secure cloud storage
    ...
    async def handle_session_command(self, client, message):
        """Handle /session command to show session info."""
        if self.use_db_storage:
            # Count sessions in database
            cursor = self.db_conn.execute('SELECT COUNT(*) FROM sessions')
            session_count = cursor.fetchone()[0]
            
            session_info = f"""
📊 **Session Information**

🗄️ **Storage Type:** Database
...
🆔 **Chat ID:** {message.chat.id}
    ...
    async def handle_logout_command(self, client, message):
        """Handle /logout command to clear session data."""
        if self.use_db_storage:
            # Delete user's session from database
            self.db_conn.execute('DELETE FROM sessions WHERE name = ?', (f"file_bot",))
            self.db_conn.commit()
            await message.reply_text("✅ Session data cleared from database.")
        else:
            # Delete file-based session
            session_file = Path("./session/file_bot.session")
            if session_file.exists():
                session_file.unlink()
                await message.reply_text("✅ Session file deleted.")
            else:
...
💡 **Tip:** This link works for direct streaming without downloading!
    ...
    async def run(self):
        ...
        # Create session directory if not using database
        if not self.use_db_storage:
            os.makedirs("session", exist_ok=True)
        
        try:
            # Use custom storage if database is enabled
            if self.use_db_storage and self.db_conn:
                # Initialize with database storage
                storage = DatabaseStorage("file_bot", self.db_conn)
                await storage.open()  # Load existing session if any
                
                # Start the client with custom storage
...
async def main():
    ...
    # Initialize bot with database session storage
    use_db_storage = os.getenv('USE_DB_STORAGE', 'false').lower() == 'true'
    bot = TelegramFileBot(use_db_storage=use_db_storage)
